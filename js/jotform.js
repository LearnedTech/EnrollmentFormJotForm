    ...
    errorCatcherLog: function (err, logTitle) {
        try {
            ...
            fetch('/server/formInitCatchLogger/' + currFormID, {
                method: 'POST',
                body: JSON.stringify({ data: err.stack || err.message, title: logTitle })
            })
        } catch (_err) {
            console.log(_err);
        }
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                ...
                element.observe('change', function (event) {
                    function deselectProduct(quantityType) {
                        ...
                        if (subProductTable && typeof subProductTable !== 'undefined') {
                            ...
                            if (quantityList.length > 0 && Array.from(quantityList).every(function(el) { return ['', '0'].includes(el.value) })) { element.classList.remove('p_selected') };
                        } else {
    ...
    */
    ...
    countTotal: function (prices) {
        ...
        if (JotForm.stripe && typeof JotForm.stripe !== undefined && window.location.search === '?paymentElements=1') {
        ...
        /**
         * Allows rounding a decimal value greater than two
         * @param num
         * @param decimalPlaces
         * @returns {number}
         */
   ...
   */
    ...
    setStripeSettings: function (pubkey, add_qid, shipping_qid, email_qid, phone_qid, custom_field_qid) {
        ...
        if (pubkey) {
            ...
            var scriptVersion = (scaTemplate && (scaTemplate.querySelector('.cc_numberMount') || scaTemplate.querySelector('#payment-element'))) ? "v3" : "v1";
        ...
        if (pages.length > 0) {
            ...
            if (last) {
                ...
                classList.filter(function(cls) {
                  if(cls.startsWith('form-submit-button-')){
    ...
    handleSignatureEvents: function() {
        var signedFields = [];
        // TODO: remove when missing signature issue resolved
            ...
            form.observe('submit', function (e) {
                try {
                    ...
                    if (JotForm.validatedRequiredFieldIDs != null && typeof JotForm.validatedRequiredFieldIDs === 'object') {
                        form.insert(new Element('input', {type: 'hidden', name: 'validatedRequiredFieldIDs'}).putValue(JSON.stringify((Object.keys(JotForm.validatedRequiredFieldIDs).length > 0 && JotForm.validatedRequiredFieldIDs) || 'No validated required fields')));
                    }
        ...
        if (question) {
          switch (question.type) {
            case 'control_chargify':
              ...
              break;
          case 'control_stripe':
              if (window.location.search !== '?paymentElements=1') {
                  break;
              }
              var email = $this.hasQuestion(questions, 'control_email');
              if (email !== false) {
